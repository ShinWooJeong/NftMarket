...
export class CronDataService {
  ...
  static async doUpdateByIdx(idx: number, params) {
    const dbData = await app().config.mysql.bluebay.cron_data.findOne({where: {idx: idx}})
  ...
  static async doDeleteByIdx(idx: number) {
    return await app().config.mysql.bluebay.cron_data.destroy({where: {idx: idx}})
  }
  ...
  static async getOneInfoByIdx(idx: number) {
    return await app().config.mysql.bluebay.cron_data.findOne({where: {idx: idx}})
  }
