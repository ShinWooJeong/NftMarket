...
export class NftHistoryService {
    ...
    static async doUpdateByIdx(idx: number, params) {
        const dbData = await app().config.mysql.bluebay.nft_history.findOne({where: {idx: idx}})
    ...
    static async doDeleteByIdx(idx: number) {
        return await app().config.mysql.bluebay.nft_history.destroy({where: {idx: idx}})
    }
    ...
    static async getOneInfoByIdx(idx: number) {
        return await app().config.mysql.bluebay.nft_history.findOne({where: {idx: idx}})
    }
